...
// called in response to echoPin rising or falling edge, doesn't use blocking pulseIn()
void ISR_SonarEcho()
{
  static uint32_t startTime;
  if (digitalRead(ECHO_PIN))
  { // pin high means start of echo (return) pulse
    startTime = micros();
  }
  else
  { // interrupt only triggered on change (rising or falling). If not rising, must be falling (echo done)
